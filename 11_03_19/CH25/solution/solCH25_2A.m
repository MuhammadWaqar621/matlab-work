...
clc
close all
% clear all work space and command window and close all figure 
...
for snr_i=1:length(EbN0dB_vector)   %repeat the loop for the length of vector 
    ...
    while err_cnt<500               %repeat the loop for count_Error less then 500
        s=sqrt(Eb)*sign(rand-0.5);  %create the s is negative or positive 
        h=sqrt(1/2)*(randn+j*randn);% create h and n by calling randn function and sqrt use
        n=sqrt(N0/2)*(randn+j*randn);
        
        r=h*s+n;                    %calculate the value r 
        
        D=r*exp(-j*angle(h));       %calculate the angle and express in exp form 
        s_hat=sign(D);        %calculate real value 
        if s_hat~=sign(s)           %if the sign  is different it error increment 
            err_cnt=err_cnt+1;
        end
